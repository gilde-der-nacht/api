...
class Olymp {
  constructor(config) {
    this.username =
      typeof config.username === "undefined" ? null : config.username;
    this.password =
      typeof config.password === "undefined" ? null : config.password;
    this.server =
  ...
  async entriesList(resourceUid) {
    ...
    const [text, status] = credentials
      ? await HTTP.getWithAuthorization(path, this.username, this.password)
      : await HTTP.get(path);
    if (status !== HTTP.CODES.OK_200) {
  ...
  async getProgram(resourceUid, secret) {
    Olymp._verify(Olymp._verifyUid(resourceUid));
    Olymp._verify(Olymp._verifyIdentification(secret));

    const path = `${this.server}/resources/${resourceUid}/program/${secret}`;
    const [text, status] = await HTTP.get(path);
    if (status !== HTTP.CODES.OK_200) {
      throw "List - Invalid Response";
    }
...
class OlympMock {
  ...
  async entriesAdd(
    ...
    const entry = {
      ...
      identification: identification,
      timestamp:
        timestamp === undefined ? OlympMock.createTimestamp() : timestamp,
      publicBody: publicBody,
      privateBody: privateBody,
      url: "",
      userAgent: "",
  ...
  async entriesList(resourceUid) {
    ...
    return OlympMock.filterMostRecent(
      unfiltered,
